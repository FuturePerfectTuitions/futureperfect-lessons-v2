...
const VIEW_META = Object.freeze({
  ...
  'english-year4-11plus': { label:'Year 4 11+ English + VR', year:4, vr:true },
  'english-year5-11plus': { label:'Year 5 11+ English + VR', year:5, vr:true }
});
...
function configured(env) {
  return Boolean(env?.STUDENTS_KV && env?.DB && env?.ADMIN_IMPORT_SESSION_SECRET);
}
...
function buildTrialRecord({ portalUserId, firstName, loginPassword, answerPassword, trialViews, existing = null }) {
  ...
  const vrEligible = views.some(viewId => VIEW_META[viewId]?.vr === true);
  return {
...
async function cleanTrialSessionState(env, portalUserIdNorm, clearConsumption) {
  const statements = [
    env.DB.prepare(
    ...
    env.DB.prepare(`DELETE FROM student_session_windows WHERE portal_user_id_norm = ?`).bind(portalUserIdNorm),
    env.DB.prepare(`DELETE FROM student_sessions WHERE portal_user_id_norm = ?`).bind(portalUserIdNorm)
  ];
  if (clearConsumption) {
    statements.push(
      env.DB.prepare(`DELETE FROM trial_login_consumptions WHERE portal_user_id_norm = ?`).bind(portalUserIdNorm)
    );
  }
...
function publicTrial(user, consumedAt = '') {
  const trialViews = normaliseTrialViews(user?.trialViews);
  ...
  return {
    ...
    trialViews,
    access:trialViews.map(viewId => VIEW_META[viewId]?.label || viewId),
    vrIncluded:trialViews.filter(viewId => VIEW_META[viewId]?.vr).map(viewId => VIEW_META[viewId].label),
    active,
...
async function handleCreate(request, env) {
  ...
  if (trialViews.length !== new Set(suppliedViews).size) {
    return json({ ok:false, error:'INVALID_TRIAL_VIEW' }, 400, request, env);
  }
  ...
  await cleanTrialSessionState(env, portalUserIdNorm, true);
  await env.STUDENTS_KV.put(key, JSON.stringify(record));
  const readback = await env.STUDENTS_KV.get(key, { type:'json' });
  const consumed = await consumptionFor(env, portalUserIdNorm);
...
async function handleList(request, env) {
  ...
  const trials = users
    .map(user => publicTrial(user, consumed.get(norm(user.portalUserId)) || ''))
...
async function handleRearm(request, env) {
  ...
  return json({ ok:true, ...publicTrial(updated), oneLoginUnused:true }, 200, request, env);
}
...
async function handleDisable(request, env) {
  ...
  await cleanTrialSessionState(env, found.idNorm, false);
  const now = new Date().toISOString();
  const updated = {
    ...found.user,
    status:'withdrawn',
