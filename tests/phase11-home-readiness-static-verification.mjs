...
assert.match(script, /\/api\/v1\/student\/session/);
assert.match(script, /bootstrapSessionPromise/);
assert.match(script, /takeBootstrapSessionResponse/);
assert.match(script, /rememberBootstrapSession/);
assert.match(script, /response\.clone\(\)/);
assert.match(script, /Loading your curriculum… Please wait\./);
assert.match(script, /function clearLoadingNotice\(\)/);
assert.match(script, /homeReady = true/);
assert.doesNotMatch(script, /holder\.textContent = 'Loading your curriculum…';/);
assert.ok(
