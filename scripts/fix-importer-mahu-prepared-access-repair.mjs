...
const batchKeys=assignments.map(x=>clean(x.batch_key)).sort();
if(stableStringify(batchKeys)!==stableStringify(['Y6FE','Y6FM']))throw new Error(`Unexpected Mahu assignments: ${JSON.stringify(batchKeys)}`);
if(ents.length!==0||pre.length!==0)throw new Error(`Unexpected Mahu access rows: entitlements=${ents.length}, prelesson=${pre.length}`);
const user=await kvJson(students,`user:${target}`);if(!user)throw new Error('Mahu profile missing');
const scopeId=await opaqueAccessScopeId(target,salt),scope=`access:${scopeId}`,beforeRaw=await kvText(readNs,pointerKey(scope));if(!beforeRaw)throw new Error('Mahu prepared pointer missing');
...
const expectedCurrent=(expected.snapshot?.views||[]).filter(v=>v.current&&!v.lockedPreview).map(v=>v.viewId).sort();
if(stableStringify(expectedCurrent)!==stableStringify(['english-year6','maths-year6']))throw new Error(`Unexpected compiled Mahu current views: ${JSON.stringify(expectedCurrent)}`);
if(stableStringify(before.payload)===stableStringify(expected))throw new Error('Mahu prepared access is already current; repair no longer required');
const beforeCurrent=(before.payload?.snapshot?.views||[]).filter(v=>v.current&&!v.lockedPreview).map(v=>v.viewId).sort();
const report={marker:'FIX_IMPORTER_MAHU_PREPARED_ACCESS_REPAIR',mode:allowWrite?'WRITE':'DRY_RUN',asOfDate,observedAt:new Date().toISOString(),portalUserId:target,scope,batchKeys,beforeVersion:before.version,beforeCurrentViews:beforeCurrent,expectedCurrentViews:expectedCurrent,status:allowWrite?'PENDING':'DRY_RUN_PASS',published:null,rollback:null};
if(!allowWrite){fs.writeFileSync('/tmp/fix-importer-mahu-repair.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));process.exit(0)}
let mutated=false;
try{
 const immediate=await kvText(readNs,pointerKey(scope));if(immediate!==beforeRaw)throw new Error('Concurrent pointer change detected for Mahu');
 const sha=await sha256Hex(stableStringify(expected)),version=`fix-mahu-${sha.slice(0,16)}-${clean(process.env.GITHUB_RUN_ID||Date.now())}`;
 ...
 const after=await resolveCurrentScope(store,scope);if(stableStringify(after.payload)!==stableStringify(expected))throw new Error('Post-publish Mahu payload mismatch');
 const afterCurrent=(after.payload?.snapshot?.views||[]).filter(v=>v.current&&!v.lockedPreview).map(v=>v.viewId).sort();
 if(stableStringify(afterCurrent)!==stableStringify(expectedCurrent))throw new Error(`Post-publish Mahu current views wrong: ${JSON.stringify(afterCurrent)}`);
 report.published={version:published.version,previousVersion:published.previousVersion,payloadSha256:published.payloadSha256};report.afterCurrentViews=afterCurrent;report.status='WRITE_PASS';report.completedAt=new Date().toISOString();
}catch(error){
 ...
 report.status='WRITE_FAILED_ROLLED_BACK';report.error=String(error?.message||error);fs.writeFileSync('/tmp/fix-importer-mahu-repair.json',JSON.stringify(report,null,2)+'\n');throw error;
}
fs.writeFileSync('/tmp/fix-importer-mahu-repair.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));
