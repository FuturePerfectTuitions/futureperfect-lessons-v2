...
ROOTS={"english":"1OBgXw-OmLuEZGkf0N8OjPKemHYzPk5ww","maths":"1Ou-XT-wUFxEgUJ2N0Ojq5P67z4s-HkWy"}
...
summary={"status":"pass","mode":"READ_ONLY_YEAR6_DIRECT_CHILD_CENSUS","roots":ROOTS,"subjects":{}}
for subject,fid in ROOTS.items():
    ...
    print("Y6_CENSUS",subject,"FOLDERS",len(folders),"FILES",len(files),flush=True)
    for x in folders: print(subject.upper(),x["name"],x["id"],flush=True)
with open("y6-drive-direct-child-census.json","w",encoding="utf-8") as f: json.dump(summary,f,indent=2,ensure_ascii=False); f.write("\n")
