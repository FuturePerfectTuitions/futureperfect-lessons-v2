...
FOLDER_ID='1OYFPtGBb9Io7dwiuIvdMYsvJnb2TczXb'
...
summary={'status':'pass','mode':'READ_ONLY_Y4_ENGLISH_DIRECT_CHILDREN','folder_id':FOLDER_ID,'count':len(out),'children':out}
for x in out:print('Y4_ENGLISH_CHILD',json.dumps({'id':x['id'],'name':x['name'],'mimeType':x.get('mimeType'),'size':x.get('size')},ensure_ascii=False,sort_keys=True),flush=True)
with open('y4-english-direct-children.json','w',encoding='utf-8') as f:json.dump(summary,f,indent=2,ensure_ascii=False,sort_keys=True);f.write('\n')
