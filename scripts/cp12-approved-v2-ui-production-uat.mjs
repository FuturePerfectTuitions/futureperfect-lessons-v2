...
const username=String(process.env.UAT_USERNAME||'admin').trim();
const password=String(process.env.UAT_LOGIN_PASSWORD||'');
const answerPassword=String(process.env.UAT_ANSWER_PASSWORD||'');
const expectedFirstName=String(process.env.UAT_EXPECTED_FIRST_NAME||'').trim().split(/\s+/)[0]||'';
...
if(!base||!username||!expectedFirstName||!expectedJs)throw new Error('Production UI UAT identity inputs are incomplete.');
if(!/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d).{4}$/.test(password))throw new Error('Production login credential shape is invalid.');
if(!/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d).{4}$/.test(answerPassword))throw new Error('Production Answer Pack credential shape is invalid.');
...
async function login(page){
  ...
  assert.equal(response.status(),200,'Production Admin login failed.');
  ...
  assert.equal(greeting,`${expectedFirstName}'s Portal`,'Personalised portal wording is not using the production first name.');
...
async function openY5E2(page,viewId){
  ...
  assert.equal(await view.count(),1,`Production Admin is missing required view ${viewId}.`);
  await view.click();
...
try{
  {
    ...
    const loginEvidence=await login(page); evidence.frontend=loginEvidence.frontend; evidence.visual.login=loginEvidence.loginPresentation; evidence.visual.personalisedPortal=loginEvidence.greeting; evidence.visual.topbarPosition=loginEvidence.topbarPosition; evidence.visual.logoutBg=loginEvidence.logoutBg;
    ...
    evidence.vr={resourceCount:vrRows.length,coreHomeworkRows:homeworkIds.size,outerSection:true,preLessonSubgroup:true,homeworkSubgroup:true,collapsedByDefault:true,viewHideToggle:true,protectedAnswers:protectedVr.length}; await context.close();
  }
  {
    const context=await browser.newContext({viewport:{width:1280,height:900}}); const page=await context.newPage(); await login(page); await assertSubjectPresentation(page);
    const detail=await openY5E2(page,'english-year5'); assert.equal((detail.resources||[]).some(row=>(row.presentationScopes||[]).includes('vr')),false,'Ordinary API response exposed VR rows.'); assert.equal(await page.locator('[data-resource-section="verbal-reasoning"]').count(),0,'Ordinary UI rendered Verbal Reasoning section.');
    ...
    const shot=path.join(evidenceDir,'desktop-ordinary-no-vr.png'); await page.screenshot({path:shot,fullPage:true}); evidence.screenshots.push(path.basename(shot)); evidence.ordinary={sameLessonControl:true,vrRowsHidden:true,vrSectionAbsent:true,homeworkCollapsible:true}; await context.close();
  }
  {
    const context=await browser.newContext({viewport:{width:390,height:844},isMobile:true}); const page=await context.newPage(); await login(page);
    ...
    const shot=path.join(evidenceDir,'mobile-approved-v2-ui.png'); await page.screenshot({path:shot,fullPage:true}); evidence.screenshots.push(path.basename(shot)); evidence.mobile={subjectCardsStack:true,noHorizontalOverflow:true,floatingBackInViewport:true}; await context.close();
  }
  ...
  evidence.security={pupilDataSynthetic:false,productionMutation:true,answerControlsRemainPasswordGated:true,viewerCannotOpenBeforePassword:true,accessNotWidened:true,subjectNavigationLocal:true};
