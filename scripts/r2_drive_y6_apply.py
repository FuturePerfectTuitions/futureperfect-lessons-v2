...
# Managed deletion scope is deliberately narrow. Historical/current ordinary English is managed;
# only current L3T...M... Maths is managed. Separate Y6M SATS/Games/Mysteries namespaces are protected.
...
summary={"status":"running","mode":"YEAR6_DRIVE_AUTHORITATIVE_GUARDED_APPLY","bucket":R2_BUCKET,"error":None,"managed_initial_keys":[],"managed_deleted_keys":[],"managed_final_keys":[]}

def sha256(b): return hashlib.sha256(b).hexdigest()
def is_managed(k): return bool(MANAGED_ENGLISH_RE.match(k) or MANAGED_MATHS_RE.match(k))
def is_year6(k): return k.startswith("english/year6/") or k.startswith("maths/year6/")
def clean_meta(o): return {x:o.get(x) for x in ("size","etag","uploaded","storageClass")}

if os.environ.get("CONFIRM_YEAR6_APPLY")!="YES": raise RuntimeError("Year 6 mutation guard is not armed")
ctx=runpy.run_path("scripts/r2_drive_y6_full_preflight.py",run_name="__y6_apply_preflight__")
p=ctx["summary"]
if p.get("status")!="pass": raise RuntimeError(f"Year 6 preflight failed: {p.get('error')}")
if p.get("ordinary_english_lesson_count")!=31 or p.get("maths_lesson_count")!=43 or p.get("lesson_count")!=74:
    raise RuntimeError(f"Year 6 lesson count mismatch english={p.get('ordinary_english_lesson_count')} maths={p.get('maths_lesson_count')} total={p.get('lesson_count')}")
...
summary["preflight_counts"]={"english_lessons":31,"maths_lessons":43,"expected_active_files":len(expected),"powerpoint_excluded":p.get("powerpoint_excluded_count"),"non_current_code_excluded":p.get("non_current_code_excluded_count"),"vr_exception_accepted":p.get("vr_exception_accepted_count")}
if (p.get("vr_exception_accepted_count") or 0) != 0:
    raise RuntimeError("Year 6 unexpectedly accepted a historical-code VR exception")
...
def main():
    ...
    protected_before={o["key"]:clean_meta(o) for o in initial if is_year6(o.get("key") or "") and not is_managed(o.get("key") or "")}
    summary["protected_unmanaged_year6_count"]=len(protected_before)
    print("Y6_APPLY_EXPECTED_COUNT",len(expected),flush=True); print("Y6_MANAGED_INITIAL_COUNT",len(managed),flush=True); print("Y6_PROTECTED_UNMANAGED_COUNT",len(protected_before),flush=True)
    for i,(k,r) in enumerate(sorted(expected.items()),1):
        ...
        if i%25==0 or i==len(expected): print("Y6_WRITE_VERIFY_PROGRESS",i,"OF",len(expected),flush=True)
    verify_all(); print("Y6_ALL_EXPECTED_REVERIFIED_BEFORE_DELETE",flush=True)
    ...
    for i,k in enumerate(unsupported,1):
        ...
        if i%25==0 or i==len(unsupported): print("Y6_DELETE_PROGRESS",i,"OF",len(unsupported),flush=True)
    ...
    if set(managed_final)!=expected_keys: raise RuntimeError(f"Final Year 6 managed key mismatch missing={sorted(expected_keys-set(managed_final))} extra={sorted(set(managed_final)-expected_keys)}")
    verify_all()
    protected_after={o["key"]:clean_meta(o) for o in final if is_year6(o.get("key") or "") and not is_managed(o.get("key") or "")}
    if protected_after!=protected_before: raise RuntimeError("Protected Year 6 non-ordinary R2 objects changed during apply")
    summary["protected_unmanaged_year6_unchanged"]=True; summary["final_active_file_count"]=len(managed_final); summary["status"]="pass"
    print("Y6_GLOBAL_EXACT_DRIVE_R2_PARITY_PASS",len(managed_final),flush=True)
try: main()
except Exception as e:
    summary["status"]="fail"; summary["error"]=f"{type(e).__name__}: {e}"; print("Y6_RECONCILIATION_FAIL",summary["error"],flush=True); raise
finally:
    with open("y6-r2-reconciliation-summary.json","w",encoding="utf-8") as f: json.dump(summary,f,indent=2,ensure_ascii=False,sort_keys=True); f.write("\n")
