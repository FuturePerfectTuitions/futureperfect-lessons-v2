...
const vrHowToViewId='special-vr-howto';
...
function withoutVrHowTo(summary){return Object.fromEntries(Object.entries(summary||{}).filter(([id])=>id!==vrHowToViewId));}
...
for(const key of (await kvKeys(studentsNs,'user:')).sort()){
  ...
  const manualSpecialBuckets=new Set((Array.isArray(user?.manualAccess?.specialBuckets)?user.manualAccess.specialBuckets:[]).map(v=>clean(v).toUpperCase()));
  const expectsVrHowTo=manualSpecialBuckets.has('VR_HOWTO');
  const row={portalUserId:id,firstName:clean(user?.firstName||user?.name),login:'NOT_RUN',home:'NOT_RUN',english:'NOT_RUN',maths:'NOT_RUN',vrHowToExpected:expectsVrHowTo,status:'UNKNOWN',differences:[]};
  ...
  if(row.home==='PASS'){
    ...
    const actualVrHowTo=actualHome[vrHowToViewId]||null;
    if(expectsVrHowTo&&(!actualVrHowTo||actualVrHowTo.lockedPreview===true))row.differences.push({kind:'AUTHORIZED_VR_HOWTO_MISSING_FROM_HOME'});
    if(!expectsVrHowTo&&actualVrHowTo)row.differences.push({kind:'UNAUTHORIZED_VR_HOWTO_PRESENT_ON_HOME'});
    const actualCompiledHome=withoutVrHowTo(actualHome);
    if(!same(expected,actualCompiledHome))row.differences.push({kind:'HOME_VIEW_MISMATCH',expectedViewIds:Object.keys(expected).sort(),actualViewIds:Object.keys(actualCompiledHome).sort()});
  }
  ...
  for(const subject of ['english','maths']){
    ...
    if(row[subject]==='PASS'){
      ...
      const actualVrHowTo=actualSubject[vrHowToViewId]||null;
      if(subject==='english'){
        if(expectsVrHowTo&&(!actualVrHowTo||actualVrHowTo.lockedPreview===true))row.differences.push({kind:'AUTHORIZED_VR_HOWTO_MISSING_FROM_ENGLISH'});
        if(!expectsVrHowTo&&actualVrHowTo)row.differences.push({kind:'UNAUTHORIZED_VR_HOWTO_PRESENT_ON_ENGLISH'});
      }else if(actualVrHowTo){
        row.differences.push({kind:'VR_HOWTO_INCORRECTLY_PRESENT_ON_MATHS'});
      }
      const actualCompiledSubject=withoutVrHowTo(actualSubject);
      if(!same(expectedSubject,actualCompiledSubject))row.differences.push({kind:`${subject.toUpperCase()}_SUBJECT_VIEW_MISMATCH`,expectedViewIds:Object.keys(expectedSubject).sort(),actualViewIds:Object.keys(actualCompiledSubject).sort()});
    }
  }
...
const out={marker:'CP12_LIVE_PUBLIC_ENTITLEMENT_ROUTE_AUDIT',generatedAt:new Date().toISOString(),readOnly:true,portalOrigin,canonicalBrowserStudent:browserStudent,currentStudentCount:results.length,passCount:results.length-failures.length,failCount:failures.length,failureIds:failures.map(x=>x.portalUserId),authorizedVrHowToIds:results.filter(x=>x.vrHowToExpected).map(x=>x.portalUserId),results};
fs.writeFileSync('/tmp/cp12-live-public-entitlement-route-audit.json',JSON.stringify(out,null,2));
console.log(JSON.stringify({marker:out.marker,currentStudentCount:out.currentStudentCount,passCount:out.passCount,failCount:out.failCount,failureIds:out.failureIds,authorizedVrHowToIds:out.authorizedVrHowToIds,results:out.results.map(({portalUserId,firstName,login,home,english,maths,vrHowToExpected,status,differences,aylaY4E1})=>({portalUserId,firstName,login,home,english,maths,vrHowToExpected,status,differences,aylaY4E1}))},null,2));
if(failures.length)throw new Error('One or more live public student entitlement routes do not match published access or VR How-To authorization.');
