(() => {
  ...
  const VIEW_LABELS = Object.freeze({
    ...
    'english-year4-11plus':'Year 4 11+ English + all Year 4 VR',
    'english-year5-11plus':'Year 5 11+ English + all Year 5 VR'
  });
  ...
  function proposedId(name) {
    const clean = String(name || '').normalize('NFKD')
      .replace(/[\u0300-\u036f]/g, '')
  ...
  function selectedViews() {
    return [...document.querySelectorAll('input[data-trial-view]:checked')]
      .map(input => input.dataset.trialView).filter(Boolean);
  }

  function accessText(views) {
    return (Array.isArray(views) ? views : []).map(viewId => VIEW_LABELS[viewId] || viewId).join(', ');
  }
  ...
  async function createTrial() {
    ...
    hideCredentials();
    status('Creating Trial login and publishing access…', 'warn');
    try {
  ...
  async function actionOnTrial(action, portalUserId) {
    const labels = {
      rearm:'give this Trial account one fresh successful login',
      disable:'disable this Trial account immediately',
      'reset-passwords':'generate new login and Answer Pack passwords'
    };
    ...
    try {
      ...
      status(
        data.oneLoginUnused
          ...
          : `Passwords reset for ${portalUserId}. Its Trial login is already used; choose Re-arm if another login is required.`,
