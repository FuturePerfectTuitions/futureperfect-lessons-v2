(() => {
  ...
  function render(results, summary) {
    ...
    for (const [k,v] of Object.entries(summary || {})) {
      const span=document.createElement('span'); span.className='pill'; span.textContent=`${k}: ${v}`; $('summary').appendChild(span);
    }
    $('resultBody').innerHTML='';
    for (const r of results) {
      const tr=document.createElement('tr');
      const vals=[r.index+2,r.portalUserId,r.name,r.batchKey,r.lessonLabel || r.lessonId,r.lessonDate,r.lessonStatus,r.releaseType,r.action || r.status,r.message || ''];
      vals.forEach((v,i)=>{ const td=document.createElement('td'); td.textContent=v ?? ''; if(i===8) td.className=r.ok===false?'err':'ok'; tr.appendChild(td); });
  ...
  $('csvFile').addEventListener('change', async () => {
    ...
    try {
      ...
      const required=['Student','Mode','Lesson','LessonDated','LessonStatus'];
      ...
      $('previewBtn').disabled=false; setStatus($('status'),`${rows.length} CSV row${rows.length===1?'':'s'} loaded. No Portal access has been changed.`,'good');
  ...
  $('previewBtn').addEventListener('click', async () => {
    ...
    setStatus($('status'),'Validating against Portal students, lessons, batches and assignments…','warn');
    try {
      ...
      if (errors) setStatus($('status'),`Preview found ${errors} validation error${errors===1?'':'s'}. Nothing has been changed. Fix the source data before importing.`,'bad');
      else if (!releasable) setStatus($('status'),'Validation passed, but there is nothing to release from these rows.','good');
      else { setStatus($('status'),`Validation passed. Review the table, then click Confirm Import to apply ${releasable} release action${releasable===1?'':'s'}.`,'good'); $('confirmBtn').disabled=false; }
    } catch(e) { setStatus($('status'),`Preview failed: ${e.message}`,'bad'); }
    finally { $('previewBtn').disabled=false; }
  });

  $('confirmBtn').addEventListener('click', async () => {
    if (!window.confirm('Apply the validated lesson releases to Portal V2?')) return;
    $('confirmBtn').disabled=true; $('previewBtn').disabled=true;
    setStatus($('status'),'Revalidating and applying releases…','warn');
    try {
      const data=await api('/api/v1/admin/lesson-releases/confirm',{ rows });
      render(data.results,data.summary);
      if (data.summary.failed) setStatus($('status'),`Import completed with ${data.summary.failed} failed row(s). Review the results below.`,'bad');
      else setStatus($('status'),`Import complete. ${data.summary.succeeded} release action${data.summary.succeeded===1?'':'s'} confirmed.`,'good');
    } catch(e) {
      if (e.data?.results) render(e.data.results,{ errors:e.data.results.filter(r=>!r.ok).length });
      setStatus($('status'), e.data?.error==='VALIDATION_FAILED' ? 'Import stopped because revalidation failed. No release actions were applied.' : `Import failed: ${e.message}`,'bad');
    } finally { $('previewBtn').disabled=false; }
  });
})();
